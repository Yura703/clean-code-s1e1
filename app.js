...
const addButton = document.querySelector('.new-task__button');//first button
...
const taskIncomplete = function() {
  ...
  //Mark task as incomplete. When the checkbox is unchecked. Append the task list item to the #incompleteTasks.
  const listItem = this.parentNode;
  incompleteTaskHolder.appendChild(listItem);
  bindTaskEvents(listItem,taskCompleted);
}
...
//The glue to hold it all together. Set the click handler to the addTask function.
addButton.addEventListener('click', function(e) {
  e.preventDefault(); // если кнопка в форме
  addTask();
  ajaxRequest();
});
...
for (let i = 0; i < completedTasksHolder.children.length; i++) {
  console.log('completedTasksHolder == ', completedTasksHolder);
